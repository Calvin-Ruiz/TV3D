...
USEDUMP is on to use dump textures or off to use png textures
    if USEDUMP is on and there is no dump textures, let's this program running a whole cycle (display all frames)
    if not, next time you launch it will only display previously-computed frames
